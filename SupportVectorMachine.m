...
[train,test] = holdout(data,80); %randomly choose
...
for c=[0.1,1,2,5,7,10]
     ...
     % bias
     bias=mean(Ys-(Xs*W'));
     %from subgradient descent
     tic;
     [W_sub_gd,bias_sub_gd]=subgradient_descent(X,Y,c);
     toc
     %from stochastic subgradient descent
     tic;
     [W_stochastic_sub_gd,bias_stochastic_sub_gd]=stochastic_subgradient_descent(X,Y,c);
     toc
     % f~ (Predicted labels)
     f=sign(Xtest*W'+bias);
     f_sub_gd=sign(Xtest*W_sub_gd'+bias_sub_gd);
     f_stochastic_sub_gd=sign(Xtest*W_stochastic_sub_gd'+bias_stochastic_sub_gd);
     % confusion matrix
     fm = confusion_mat(Ytest,f);
     fm_= [fm_; c fm];    
     fm_sub_gd = confusion_mat(Ytest,f_sub_gd);
     fm_sub_gd_ = [fm_sub_gd_; c fm_sub_gd]; 
     fm_stochastic_sub_gd = confusion_mat(Ytest,f_stochastic_sub_gd);
     fm_stochastic_sub_gd_ = [fm_stochastic_sub_gd_; c fm_stochastic_sub_gd]; 
end
...
primal=c_optimal*(zeta'*ones(size(X,1),1))+ 0.5*norm(W,2)^2
% Duality Gap
